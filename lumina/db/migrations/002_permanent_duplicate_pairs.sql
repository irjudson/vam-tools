-- Migration: Create permanent duplicate_pairs table
--
-- Replaces temporary duplicate_pairs_temp table with permanent storage
-- Enables incremental comparison and historical duplicate tracking
...
-- ============================================================================
-- Note: This migration will preserve existing comparison results from temp table

-- First, check if temp table exists and migrate data
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'duplicate_pairs_temp') THEN
        -- Migrate data from temp table, ensuring consistent ordering
        INSERT INTO duplicate_pairs (catalog_id, image_1, image_2, hash_type, distance, job_id)
        SELECT
            c.id as catalog_id,
            CASE WHEN dpt.image_1 < dpt.image_2 THEN dpt.image_1 ELSE dpt.image_2 END as image_1,
            CASE WHEN dpt.image_1 < dpt.image_2 THEN dpt.image_2 ELSE dpt.image_1 END as image_2,
            dpt.type as hash_type,
            dpt.distance,
            dpt.job_id
        FROM duplicate_pairs_temp dpt
        INNER JOIN jobs j ON dpt.job_id = j.id
        INNER JOIN catalogs c ON j.catalog_id = c.id
        ON CONFLICT (catalog_id, image_1, image_2, hash_type) DO UPDATE
            SET distance = EXCLUDED.distance,
                compared_at = NOW(),
                job_id = EXCLUDED.job_id;

        RAISE NOTICE 'Migrated % rows from duplicate_pairs_temp to duplicate_pairs',
